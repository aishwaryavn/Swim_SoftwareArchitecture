...
# Generate all possible states and add them to a list

# we are considering three parameters to determine the state of the agent: number of servers, dimmer value and arrival rate

state_string = 'hml'
state_string_list = [[]]
length = len(state_string)
groups = [list(state_string)] * length
for i in groups:
    state_string_list = [x+[y] for x in state_string_list for y in i]
state_list = [''.join(item) for item in state_string_list]
...
class Env():
    ...
    def __init__(self):
        self.qos_states = 27 # The MXM matrix for state action space
        self.columns = 10
    ...
    def step(self,action,response_time,dimmer_value, U_rt,U_ct,U_rt_star,arrival_rate,server_count):
        ...
        if (arrival_rate>=100):
            state_string = state_string + "h"
        elif (arrival_rate>=50 and arrival_rate <100):
            state_string = state_string + "m"
        elif (arrival_rate<50):
            state_string =  state_string + "l"

        if (dimmer_value >= 1.0):
            state_string = state_string + "h"
        elif (dimmer_value >=0.5 and dimmer_value <1.0):
            state_string = state_string + "m"
        ...
        if (server_count>=3):
            state_string = state_string + "h"
        elif (server_count >=2 and server_count<3):
            state_string =  state_string + "m"
        elif (server_count<2):
            state_string = state_string + "l"

        print (state_string)
        state =  state_list.index(state_string)
        print (" state ", state)
        logger.info(" Current state " + str(state_string))
    ...
    def random_action(self,server_add_flag,server_remove_flag, current_server_count,dimmer_increase_flag, dimmer_decrease_flag):
        ...
        if server_remove_flag is True or server_add_flag is True:
            ...
            possible_choice = []
            if dimmer_decrease_flag is True and dimmer_increase_flag is True:
                possible_choice = [2, 3, 4]
            elif dimmer_increase_flag is False:
                possible_choice = [3, 4]
            elif dimmer_decrease_flag is False:
                possible_choice = [2, 4]
            ...
            if dimmer_decrease_flag is True and dimmer_increase_flag is True:
                ...
                possible_choice = [1, 3, 4, 8]
            elif dimmer_decrease_flag is False:
                possible_choice = [1, 2, 4, 7]
            return np.random.choice(possible_choice)
            ...
            if dimmer_decrease_flag is True and dimmer_increase_flag is True:
                ...
                possible_choice = [0, 3, 4, 6]
            elif dimmer_decrease_flag is False:
                possible_choice = [0, 1, 2, 4, 5]
            return np.random.choice(possible_choice)
        else:
            ...
            if dimmer_decrease_flag is True and dimmer_increase_flag is True:
                ...
                possible_choice = [0, 1, 3, 4, 6, 8]
            elif dimmer_decrease_flag is False:
                possible_choice = [0, 1, 2, 4, 5, 7]
            return np.random.choice(possible_choice)
...
class Reinforcer():
    ...
    def __init__(self):
        ...
        self.qTable = np.load("Qtable_Test.npy").tolist()
        # self.qTable = np.load("Qtable_Reactive_RLMC_v1.npy").tolist()
        self.state = 2
    ...
    def select_action(self,mode,response_time,dimmer_value,U_rt,U_ct,U_rt_star,arrival_rate,current_server_count,server_add_flag, server_remove_flag,verification_flag=False,dimmer_increase_flag=True,dimmer_decrease_flag=True):
        ...
        # dimmer flags denote that the dimmer values can be increased or decreased. If either is false, then corresponding action cannot be performed
        self.count+=1
        self.count = 5
        ...
        if self.count<=1 or mode == "ADA":
            ...
            next_state, reward = env.step(self,response_time, dimmer_value, U_rt, U_ct, U_rt_star, arrival_rate,current_server_count)
            ...
            np.save("Qtable_Test.npy", self.qTable)
            self.state = next_state


        if np.random.uniform() < self.epsilon:
            #Allow little random exploration
            action = env.random_action(server_add_flag,server_remove_flag,current_server_count,dimmer_increase_flag,dimmer_decrease_flag)
        else:
            if server_remove_flag is True or server_add_flag is True:
                # Only change of dimmer and no other action can be performed
...
if __name__ == '__main__':
    ...
    reinforce_object.select_action("ADA", 0.10,0.50,15.0,12.0,12.5,120,1, True, False)
